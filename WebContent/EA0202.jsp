...
<c:choose>
	...
	<c:when test="${!fn:contains(applicationScope.menu[sessionScope.role],'退休复学申请审批')}">
		...
		<tr>
	     ...
	     <c:when test="${rows!=null }">
		     ...
		     <c:forEach items="${rows }" var="ins" varStatus="vs">
				    ...
				    <td>
				    	<button class="btn btn-primary mr-2" onclick="onDetails('${ins.aab201}')">详情</button>
				    </td>
				    <c:choose>
					    <c:when test="${ins.aab207==1 }">
				    		<td>
					        	<button class="btn btn-success mr-2" onclick="onAgree('${ins.aab201}')">同意</button>    
				    		</td>
				    		<td>
					        	<button class="btn btn-danger mr-2" onclick="onReject('${ins.aab201}')">驳回</button>
				    		</td>
					    </c:when>
					    <c:otherwise>
					   	 	<td colspan="2">已审批</td>
