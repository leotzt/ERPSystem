			...
			<form id="myform" action="<%=path%>/EB0201QueryAFRequest.html" method="post">
				  ...
				  <tr>
				    ...
				    <td style="width:120px">
				      <e:select styleClass="form-control" name="qaac508" header="true" value="教研经费:1,活动经费:2,后勤经费:3"  style="width:100%"/>
				    </td>
			      </tr>
			      <tr>
				    <td>开始日期</td>
				    ...
				    <td>审核状态</td>
				    <td style="width:120px">
				      <e:select styleClass="form-control" name="qaac505" header="true" value="已通过:4,已拨出:5,已结算:6"  style="margin-right:30px;width:100%;" defval="${ins.aac505}"/>
				    </td>
				  </tr>
				</table>
					     ...
					     <c:when test="${rows!=null }">
						     ...
						     <c:forEach items="${rows }" var="ins" varStatus="vs">
								    ...
								    <td>${ins.fvalue}</td>
								     <td>
								    <!-- #  空锚 -->
								      <button class="btn btn-primary mr-2"  onclick="onEdit('${ins.aac501}','${ins.aac505 }','${ins.aac507 }')">详情</button>
								      </td>
								      <c:choose>
								     <c:when test="${ins.aac505==4}">
								          <td><button class="btn btn-success mr-2"  onclick="onGive('${ins.aac501}','${ins.aae101 }','${ins.aac503 }','${ins.aac508 }')">拨出</button></td>
								          <td></td>
								     </c:when> 
								     <c:when test="${ins.aac505>4 }">
										     <td>已拨出 </td>
										     ...
										     <c:choose>
										         <c:when test="${ins.aac507>2 }">
										             <button class="btn btn-info mr-2" onclick="onEnd('${ins.aac501}','${ins.aac505 }','${ins.aac507 }','${ins.aae101 }')">${ins.aac507==3?'结算':'记录' }</button>
										         </c:when>
										         <c:otherwise>
										             未提交记录
										         </c:otherwise>
