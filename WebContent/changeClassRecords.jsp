...
<script type="text/javascript">
	function onCancel(vaab401)
	{
		if(confirm("确定要取消此条申请吗"))
		{
			var vform = document.getElementById("changeClass-records-form");
		  	vform.action="<%=path%>/delClassRecordsById.html?aab401="+vaab401;
		  	vform.submit();
		}
	}
</script>
...
<form id="changeClass-records-form" action="<%=path%>/changeClassRecords.html" method="post">
		     ...
		     <c:forEach items="${rows }" var="ins" varStatus="vs">
				    ...
				    <c:choose>
					    <c:when test="${ins.aab407==1 }">
					        <button class="btn btn-danger mr-2" onclick="onCancel('${ins.aab401}')">取消</button>
					    </c:when>
					    <c:otherwise>
					    	不能修改
					    </c:otherwise>
	...
	<div style="width:100%;margin-top: 25px;float:left" align="center" >
		<div class="form-group" align="center" >	 
              ...
              formaction="<%=path%>/changeClass.jsp">添加</button>
		</div>
	</div>
